...
function compileToFunction(message, options = {}) {
  ...
  {
    const warnHtmlMessage = isBoolean2(options.warnHtmlMessage) ? (
    ...
    const { code: code3, detectError } = baseCompile2(message, options);
...
var isMessageAST = (val) => isObject2(val) && val.type === 0 && "body" in val;
function translate(context, ...args) {
  ...
  const msg = !isMessageFunction(format5) ? compileMessageFormat(context, key, targetLocale, format5, cacheBaseKey, errorDetector) : format5;
  if (occurred) {
    return format5;
  }
...
function compileMessageFormat(context, key, targetLocale, format5, cacheBaseKey, errorDetector) {
  ...
  const msg = messageCompiler(format5, getCompileOptions(context, targetLocale, cacheBaseKey, format5, warnHtmlMessage, errorDetector));
  if (inBrowser2) {
    const end = window.performance.now();
    const emitter = context.__v_emitter;
    if (emitter && start) {
