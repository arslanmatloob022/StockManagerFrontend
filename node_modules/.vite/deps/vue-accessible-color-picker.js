import "./chunk-CG7YWYQK.js";
import {
  Fragment,
  computed,
  createBaseVNode,
  createCommentVNode,
  createElementBlock,
  createTextVNode,
  defineComponent,
  onBeforeUnmount,
  onMounted,
  openBlock,
  reactive,
  ref,
  renderList,
  renderSlot,
  toDisplayString,
  watch
} from "./chunk-GPRR4DFU.js";
import "./chunk-WGAPYIUP.js";

// node_modules/.pnpm/vue-accessible-color-picker@5.0.1_vue@3.4.26_typescript@5.4.5_/node_modules/vue-accessible-color-picker/dist/ColorPicker.js
function $(t, e, s) {
  return Math.max(e, Math.min(t, s));
}
function xt(t, e) {
  if (typeof t == "string" || typeof e == "string")
    return t === e;
  for (const s in t)
    if (t[s] !== e[s])
      return false;
  return true;
}
function N(t) {
  const e = [], s = t.length > 5 ? 2 : 1;
  for (let o = 1; o < t.length; o += s) {
    const u = t.substring(o, o + s).repeat(s % 2 + 1), f = parseInt(u, 16);
    e.push(o === 3 * s + 1 ? f / 255 : f);
  }
  return e.length === 3 && e.push(1), {
    r: e[0],
    g: e[1],
    b: e[2],
    a: e[3]
  };
}
function Q(t) {
  const e = t.l / 100, s = e + t.s / 100 * Math.min(e, 1 - e), o = s === 0 ? 0 : 200 * (1 - e / s);
  return {
    h: t.h,
    s: o,
    v: s * 100,
    a: t.a
  };
}
function A(t) {
  let e = t.h % 360;
  e < 0 && (e += 360);
  const s = t.s / 100, o = t.l / 100;
  return {
    r: S(0, e, s, o) * 255,
    g: S(8, e, s, o) * 255,
    b: S(4, e, s, o) * 255,
    a: t.a
  };
}
function S(t, e, s, o) {
  const u = (t + e / 30) % 12, f = s * Math.min(o, 1 - o);
  return o - f * Math.max(-1, Math.min(u - 3, 9 - u, 1));
}
function Z(t) {
  const e = t.s / 100, s = t.v / 100, o = s * (1 - e / 2);
  return {
    h: t.h,
    s: o === 0 || o === 1 ? 0 : (s - o) / Math.min(o, 1 - o) * 100,
    l: o * 100,
    a: t.a
  };
}
function tt(t) {
  return {
    h: t.h,
    w: t.v * (100 - t.s) / 100,
    b: 100 - t.v,
    a: t.a
  };
}
function _(t) {
  return A(Z(t));
}
function x(t) {
  const e = t.w / 100, s = t.b / 100;
  let o, u;
  const f = e + s;
  return f >= 1 ? (o = 0, u = e / f) : (u = 1 - s, o = (1 - e / u) * 100), {
    h: t.h,
    s: o,
    v: u * 100,
    a: t.a
  };
}
function H(t) {
  const { r: e, g: s, b: o, a: u } = t, f = Math.min(e, s, o), v = Math.max(e, s, o), p = v - f, d = (v + f) / 2;
  let c = 0;
  p !== 0 && (v === e ? c = (s - o) / p + (s < o ? 6 : 0) : v === s ? c = (o - e) / p + 2 : v === o && (c = (e - s) / p + 4), c *= 60);
  let l = 0;
  return d !== 0 && d !== 255 && (l = (v - d) / Math.min(d, 255 - d)), {
    h: c,
    s: l * 100,
    l: d / 255 * 100,
    a: u
  };
}
function V(t) {
  return "#" + Object.values(t).map((e, s) => Math.round(s === 3 ? e * 255 : e).toString(16).padStart(2, "0")).join("");
}
function M(t) {
  return tt(Q(H(t)));
}
var Ct = {
  hex: {
    hex: (t) => t,
    hsl: (t) => H(N(t)),
    hsv: (t) => x(M(N(t))),
    hwb: (t) => M(N(t)),
    rgb: N
  },
  hsl: {
    hex: (t) => V(A(t)),
    hsl: (t) => t,
    hsv: Q,
    hwb: (t) => M(A(t)),
    rgb: A
  },
  hsv: {
    hex: (t) => V(_(t)),
    hsl: Z,
    hsv: (t) => t,
    hwb: tt,
    rgb: _
  },
  hwb: {
    hex: (t) => V(_(x(t))),
    hsl: (t) => H(_(x(t))),
    hsv: x,
    hwb: (t) => t,
    rgb: (t) => _(x(t))
  },
  rgb: {
    hex: V,
    hsl: H,
    hsv: (t) => x(M(t)),
    hwb: M,
    rgb: (t) => t
  }
};
function Tt(t, e, s) {
  return Ct[t][e](s);
}
function _t(t, e) {
  const s = t.toFixed(e);
  return s.includes(".") ? s.replace(/\.?0+$/, "") : s;
}
var Mt = {
  deg: 1,
  grad: 0.9,
  rad: 180 / Math.PI,
  turn: 360
};
var R = {
  from(t) {
    return t.endsWith("%") ? C.from(t, { referenceValue: 1 }) : g.from(t, { min: 0, max: 1 });
  },
  to(t) {
    return g.to(t);
  }
};
var G = {
  from(t) {
    const e = t.match(/deg|g?rad|turn$/);
    if (e === null)
      return g.from(t);
    const s = e[0];
    return g.from(t.slice(0, -s.length)) * Mt[s];
  },
  to(t) {
    return g.to(t);
  }
};
var g = {
  from(t, { min: e = Number.NEGATIVE_INFINITY, max: s = Number.POSITIVE_INFINITY } = {}) {
    return t.endsWith(".") ? NaN : $(Number(t), e, s);
  },
  to(t) {
    return _t(t, 2);
  }
};
var C = {
  from(t, { referenceValue: e = 100, min: s = 0, max: o = 100 } = {}) {
    return t.endsWith("%") ? g.from(t.slice(0, -1), { min: s, max: o }) * e / 100 : NaN;
  },
  to(t) {
    return g.to(t) + "%";
  }
};
var j = {
  from(t) {
    return t.endsWith("%") ? C.from(t, { referenceValue: 255 }) : g.from(t, { min: 0, max: 255 });
  },
  to(t) {
    return g.to(t);
  }
};
var $t = {
  hsl: {
    h: G,
    s: C,
    l: C,
    a: R
  },
  hwb: {
    h: G,
    w: C,
    b: C,
    a: R
  },
  rgb: {
    r: j,
    g: j,
    b: j,
    a: R
  }
};
function E(t, e) {
  return $t[t][e];
}
function W({ format: t, color: e }, s) {
  if (t === "hex")
    return s && [5, 9].includes(e.length) ? e.substring(0, e.length - (e.length - 1) / 4) : e;
  const o = Object.entries(e).slice(0, s ? 3 : 4).map(([u, f]) => {
    const v = E(t, u);
    return (u === "a" ? "/ " : "") + v.to(f);
  });
  return `${t}(${o.join(" ")})`;
}
function et(t) {
  return /^#(?:(?:[A-F0-9]{2}){3,4}|[A-F0-9]{3,4})$/i.test(t);
}
function kt(t) {
  return "r" in t ? "rgb" : "w" in t ? "hwb" : "v" in t ? "hsv" : "s" in t ? "hsl" : null;
}
var J = {
  hsl: ["h", "s", "l", "a"],
  hwb: ["h", "w", "b", "a"],
  rgb: ["r", "g", "b", "a"]
};
function Ft(t) {
  if (typeof t != "string") {
    const p = kt(t);
    return p === null ? null : { format: p, color: t };
  }
  if (t.startsWith("#"))
    return et(t) ? { format: "hex", color: t } : null;
  if (!t.includes("(")) {
    const p = document.createElement("canvas").getContext("2d");
    p.fillStyle = t;
    const d = p.fillStyle;
    return d === "#000000" && t !== "black" ? null : { format: "hex", color: d };
  }
  const [e, s] = t.split("("), o = e.substring(0, 3);
  if (!(o in J))
    return null;
  const u = s.replace(/[,/)]/g, " ").replace(/\s+/g, " ").trim().split(" ");
  u.length === 3 && u.push("1");
  const f = J[o], v = Object.fromEntries(f.map((p, d) => {
    const c = E(o, p);
    return [
      p,
      c.from(u[d])
    ];
  }));
  return { format: o, color: v };
}
function It(t, e, s) {
  const o = t.getBoundingClientRect(), u = e - o.left, f = s - o.top;
  return {
    x: o.width === 0 ? 0 : $(u / o.width * 100, 0, 100),
    y: o.height === 0 ? 0 : $((1 - f / o.height) * 100, 0, 100)
  };
}
var Nt = { class: "vacp-range-input-group" };
var Vt = ["for"];
var At = { class: "vacp-range-input-label-text vacp-range-input-label-text--hue" };
var Ht = ["id", "value"];
var Et = ["for"];
var Lt = { class: "vacp-range-input-label-text vacp-range-input-label-text--alpha" };
var Pt = ["id", "value"];
var Ot = createBaseVNode("span", { class: "vacp-visually-hidden" }, "Copy color", -1);
var St = createBaseVNode("svg", {
  class: "vacp-icon",
  xmlns: "http://www.w3.org/2000/svg",
  "aria-hidden": "true",
  width: "24",
  height: "24",
  viewBox: "0 0 32 32"
}, [
  createBaseVNode("path", {
    d: "M25.313 28v-18.688h-14.625v18.688h14.625zM25.313 6.688c1.438 0 2.688 1.188 2.688 2.625v18.688c0 1.438-1.25 2.688-2.688 2.688h-14.625c-1.438 0-2.688-1.25-2.688-2.688v-18.688c0-1.438 1.25-2.625 2.688-2.625h14.625zM21.313 1.313v2.688h-16v18.688h-2.625v-18.688c0-1.438 1.188-2.688 2.625-2.688h16z",
    fill: "currentColor"
  })
], -1);
var Rt = { class: "vacp-color-inputs" };
var jt = { class: "vacp-color-input-group" };
var Wt = ["for"];
var Dt = createBaseVNode("span", { class: "vacp-color-input-label-text" }, " Hex ", -1);
var zt = ["id", "value"];
var Kt = ["id", "for", "onInput"];
var Yt = { class: "vacp-color-input-label-text" };
var Bt = ["id", "value", "onInput"];
var Ut = createBaseVNode("span", { class: "vacp-visually-hidden" }, "Switch format", -1);
var qt = createBaseVNode("svg", {
  class: "vacp-icon",
  "aria-hidden": "true",
  xmlns: "http://www.w3.org/2000/svg",
  width: "16",
  height: "15"
}, [
  createBaseVNode("path", {
    d: "M8 15l5-5-1-1-4 2-4-2-1 1zm4-9l1-1-5-5-5 5 1 1 4-2z",
    fill: "currentColor"
  })
], -1);
var Xt = defineComponent({
  __name: "ColorPicker",
  props: {
    color: { default: "#ffffffff" },
    id: { default: "color-picker" },
    visibleFormats: { default: () => ["hex", "hsl", "hwb", "rgb"] },
    defaultFormat: { default: "hsl" },
    alphaChannel: { default: "show" }
  },
  emits: ["color-change"],
  setup(t, { emit: e }) {
    const s = ["hex", "hsl", "hsv", "hwb", "rgb"], o = t, u = e, f = ref(null), v = ref(null), p = ref(null);
    let d = false;
    const c = ref(o.visibleFormats.includes(o.defaultFormat) ? o.defaultFormat : o.visibleFormats[0]), l = reactive({
      hex: "#ffffffff",
      hsl: { h: 0, s: 0, l: 100, a: 1 },
      hsv: { h: 0, s: 0, v: 100, a: 1 },
      hwb: { h: 0, w: 100, b: 0, a: 1 },
      rgb: { r: 255, g: 255, b: 255, a: 1 }
    }), nt = computed(function() {
      const n = Object.keys(l[c.value]);
      return c.value !== "hex" && o.alphaChannel === "hide" ? n.slice(0, 3) : n;
    }), ot = computed(function() {
      return o.alphaChannel === "hide" && [5, 9].includes(l.hex.length) ? l.hex.substring(0, l.hex.length - (l.hex.length - 1) / 4) : l.hex;
    });
    watch(() => o.color, z), onMounted(function() {
      document.addEventListener("mousemove", L, { passive: false }), document.addEventListener("touchmove", P, { passive: false }), document.addEventListener("mouseup", k), document.addEventListener("touchend", k), z(o.color);
    }), onBeforeUnmount(function() {
      document.removeEventListener("mousemove", L), document.removeEventListener("touchmove", P), document.removeEventListener("mouseup", k), document.removeEventListener("touchend", k);
    });
    function st() {
      const r = (o.visibleFormats.findIndex((a) => a === c.value) + 1) % o.visibleFormats.length;
      c.value = o.visibleFormats[r];
    }
    function rt(n) {
      d = true, L(n);
    }
    function at(n) {
      d = true, P(n);
    }
    function k() {
      d = false;
    }
    function L(n) {
      n.buttons !== 1 || d === false || !(v.value instanceof HTMLElement) || D(v.value, n.clientX, n.clientY);
    }
    function P(n) {
      if (d === false || !(v.value instanceof HTMLElement))
        return;
      n.preventDefault();
      const r = n.touches[0];
      D(v.value, r.clientX, r.clientY);
    }
    function D(n, r, a) {
      const i = It(n, r, a), m = Object.assign({}, l.hsv);
      m.s = i.x, m.v = i.y, b("hsv", m);
    }
    function lt(n) {
      if (!["ArrowUp", "ArrowRight", "ArrowDown", "ArrowLeft"].includes(n.key))
        return;
      n.preventDefault();
      const r = ["ArrowLeft", "ArrowDown"].includes(n.key) ? -1 : 1, a = ["ArrowLeft", "ArrowRight"].includes(n.key) ? "s" : "v", i = n.shiftKey ? 10 : 1, m = l.hsv[a] + r * i, T = Object.assign({}, l.hsv);
      T[a] = $(m, 0, 100), b("hsv", T);
    }
    function z(n) {
      const r = Ft(n);
      r !== null && b(r.format, r.color);
    }
    function K(n, r) {
      const a = n.currentTarget, i = Object.assign({}, l.hsv);
      i[r] = Number(a.value), b("hsv", i);
    }
    function it(n) {
      const r = n.target;
      et(r.value) && b("hex", r.value);
    }
    function Y(n, r) {
      const a = n.target, i = c.value, m = Object.assign({}, l[i]), O = E(i, r).from(a.value);
      Number.isNaN(O) || O === void 0 || (m[r] = O, b(i, m));
    }
    function b(n, r) {
      let a = r;
      if (o.alphaChannel === "hide")
        if (typeof r != "string")
          r.a = 1, a = r;
        else if ([5, 9].includes(r.length)) {
          const i = (r.length - 1) / 4;
          a = r.substring(0, r.length - i) + "f".repeat(i);
        } else
          [4, 7].includes(r.length) && (a = r + "f".repeat((r.length - 1) / 3));
      if (!xt(l[n], a)) {
        l[n] = a;
        for (const i of s)
          i !== n && (l[i] = Tt(n, i, a));
        u("color-change", ft());
      }
      f.value instanceof HTMLElement && v.value instanceof HTMLElement && p.value instanceof HTMLElement && ht(f.value, v.value, p.value);
    }
    async function ut() {
      const n = l[c.value], r = o.alphaChannel === "hide", a = W({ color: n, format: c.value }, r);
      await window.navigator.clipboard.writeText(a);
    }
    function ct(n) {
      const r = c.value;
      return E(r, n).to(l[r][n]);
    }
    function ht(n, r, a) {
      const i = W({ format: "hsl", color: l.hsl }, false);
      n.style.setProperty("--vacp-color", i), r.style.position = "relative", r.style.backgroundColor = `hsl(${l.hsl.h} 100% 50%)`, r.style.backgroundImage = "linear-gradient(to top, #000, transparent), linear-gradient(to right, #fff, transparent)", a.style.boxSizing = "border-box", a.style.position = "absolute", a.style.left = `${l.hsv.s}%`, a.style.bottom = `${l.hsv.v}%`;
    }
    function ft() {
      const n = o.alphaChannel === "hide", r = W({ color: l[c.value], format: c.value }, n);
      return {
        colors: l,
        cssColor: r
      };
    }
    function B(n) {
      if (!["ArrowUp", "ArrowRight", "ArrowDown", "ArrowLeft"].includes(n.key) || !n.shiftKey)
        return;
      const r = n.currentTarget, a = Number(r.step), i = ["ArrowLeft", "ArrowDown"].includes(n.key) ? -1 : 1, m = Number(r.value) + i * a * 10, T = $(m, Number(r.min), Number(r.max));
      r.value = String(T - i * a);
    }
    return (n, r) => (openBlock(), createElementBlock("div", {
      ref_key: "colorPicker",
      ref: f,
      class: "vacp-color-picker"
    }, [
      createBaseVNode("div", {
        ref_key: "colorSpace",
        ref: v,
        class: "vacp-color-space",
        onMousedown: rt,
        onTouchstart: at
      }, [
        createBaseVNode("div", {
          ref_key: "thumb",
          ref: p,
          class: "vacp-color-space-thumb",
          tabindex: "0",
          "aria-label": "Color space thumb",
          onKeydown: lt
        }, null, 544)
      ], 544),
      createBaseVNode("div", Nt, [
        createBaseVNode("label", {
          class: "vacp-range-input-label vacp-range-input-label--hue",
          for: `${n.id}-hue-slider`
        }, [
          createBaseVNode("span", At, [
            renderSlot(n.$slots, "hue-range-input-label", {}, () => [
              createTextVNode("Hue")
            ])
          ]),
          createBaseVNode("input", {
            id: `${n.id}-hue-slider`,
            class: "vacp-range-input vacp-range-input--hue",
            value: l.hsv.h,
            type: "range",
            min: "0",
            max: "360",
            step: "1",
            onKeydownPassive: B,
            onInput: r[0] || (r[0] = (a) => K(a, "h"))
          }, null, 40, Ht)
        ], 8, Vt),
        n.alphaChannel === "show" ? (openBlock(), createElementBlock("label", {
          key: 0,
          class: "vacp-range-input-label vacp-range-input-label--alpha",
          for: `${n.id}-alpha-slider`
        }, [
          createBaseVNode("span", Lt, [
            renderSlot(n.$slots, "alpha-range-input-label", {}, () => [
              createTextVNode("Alpha")
            ])
          ]),
          createBaseVNode("input", {
            id: `${n.id}-alpha-slider`,
            class: "vacp-range-input vacp-range-input--alpha",
            value: l.hsv.a,
            type: "range",
            min: "0",
            max: "1",
            step: "0.01",
            onKeydownPassive: B,
            onInput: r[1] || (r[1] = (a) => K(a, "a"))
          }, null, 40, Pt)
        ], 8, Et)) : createCommentVNode("", true)
      ]),
      createBaseVNode("button", {
        class: "vacp-copy-button",
        type: "button",
        onClick: ut
      }, [
        renderSlot(n.$slots, "copy-button", {}, () => [
          Ot,
          St
        ])
      ]),
      createBaseVNode("div", Rt, [
        createBaseVNode("div", jt, [
          c.value === "hex" ? (openBlock(), createElementBlock("label", {
            key: 0,
            class: "vacp-color-input-label",
            for: `${n.id}-color-hex`
          }, [
            Dt,
            createBaseVNode("input", {
              id: `${n.id}-color-hex`,
              class: "vacp-color-input",
              type: "text",
              value: ot.value,
              onInput: it
            }, null, 40, zt)
          ], 8, Wt)) : (openBlock(true), createElementBlock(Fragment, { key: 1 }, renderList(nt.value, (a) => (openBlock(), createElementBlock("label", {
            id: `${n.id}-color-${c.value}-${a}-label`,
            key: `${n.id}-color-${c.value}-${a}-label`,
            class: "vacp-color-input-label",
            for: `${n.id}-color-${c.value}-${a}`,
            onInput: (i) => Y(i, a)
          }, [
            createBaseVNode("span", Yt, toDisplayString(a.toUpperCase()), 1),
            createBaseVNode("input", {
              id: `${n.id}-color-${c.value}-${a}`,
              class: "vacp-color-input",
              type: "text",
              value: ct(a),
              onInput: (i) => Y(i, a)
            }, null, 40, Bt)
          ], 40, Kt))), 128))
        ]),
        n.visibleFormats.length > 1 ? (openBlock(), createElementBlock("button", {
          key: 0,
          class: "vacp-format-switch-button",
          type: "button",
          onClick: st
        }, [
          renderSlot(n.$slots, "format-switch-button", {}, () => [
            Ut,
            qt
          ])
        ])) : createCommentVNode("", true)
      ])
    ], 512));
  }
});
var Jt = {
  install(t) {
    t.component("ColorPicker", Xt);
  }
};
export {
  Xt as ColorPicker,
  Jt as default
};
//# sourceMappingURL=vue-accessible-color-picker.js.map
